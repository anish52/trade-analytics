...
class Widgets(object):
    """
    This class generates all the widgets required for the dashboard.
    Each widget is generated by a separate function definiton as listed below:

    ---------------------------------------------------------------------
    Methods:
        - def get_banner_widget():          Banner Widget
        - def stock_price_plot_factory() :  Stock Price Widget
        - def ti_plot_factory():            Techincal Indicators(TI) Widget
        - def get_heatmap_widget():         Heatmap Widget for the daily stock performance
        - def get_index_stats_widget():     Index Stats widget
        - def get_sector_wise_widget():     Sector Wise Performance Widget
        - def get_cube_widget():            Cube Widget for Sector Wise Performance
            
        * The parameters have not been included here. Please refer to the respective functions
          to see their usage.
    ...
    def get_banner_widget(self):
        '''
        This method generates the banner and returns it as ipywidget.
        '''
    ...
    def stock_price_plot_factory(self, stock_list, data_daily):
        '''
        This function generates the stock price graph/indicator ipywidget.
        This is an interactive widget which reflects real-time data 
        from yfinance. It includes dropdown menus to select among a list 
        of stocks and the time-interval.

        ---------------------------------------------------------------------
        Attributes:
            stock_list: List of stocks whose prices are to be visualized
            data_daily: DataFrame on different stocks' prices on daily-interval
        '''
        fig = go.Figure()
    ...
    def ti_plot_factory(self, data_list, ti_list, time_intervals=['1d', '30m', '5m'],\
        ...
        '''
        This function generates the techincal indicator ipywidget.
        This is an interactive widget which reflects real-time data. 
        It includes dropdown menus to select among a list 
        of stocks and the time-interval.
        ...
        Attributes:
            ...
            curr_interval: the pre-selected interval when the widget shows up for the ifrst time
            cur_stock: the pre-selected stock when the widget shows up for the ifrst time
        '''
    ...
    def get_heatmap_widget(self, parent_path='./output/'):
        '''
        This function generates the heatmap ipywidget presented on the
        home screen. This widget reflects real-time daily performance of 
        pre-decided stocks based on previous day's closing price. 
        
        ---------------------------------------------------------------------
        Attributes:
            parent_path: path to the png file which is rendered as widget
        '''
    ...
    def get_index_stats_widget(self, sector_market_share, percent_change_df, data = ["^DJI","^IXIC", "^GSPC", "QQQ"]):
        '''
        This function generates the index stats ipywidget presented
        on the home screen. It also includes the best performing sector
        for the day.
        ---------------------------------------------------------------------
        Attributes:
            sector_market_share: Sector-wise major stocks and their market cap
            percent_change_df: DataFrame with the precentage change in each sector
            data: list of 'index' to be displayed on the home screen
        '''
    ...
    def get_sector_wise_widget(self, percent_change_df):
        '''
        This function generates the sector-wise performance ipywidget.
        ---------------------------------------------------------------------
        Attributes:
            percent_change_df: DataFrame with the precentage change in each sector
        '''
    ...
    def get_cube_widget(self, percent_change_df):
        '''
        This function generates an animated sector-wise performance cube widget.
        ------------------------------------------------------------------------
        Attributes:
            percent_change_df: DataFrame with the precentage change in each sector
        '''
